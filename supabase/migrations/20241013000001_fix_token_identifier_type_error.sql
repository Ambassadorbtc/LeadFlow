-- Fix token_identifier type error in previous migrations
...
BEGIN
  ...
  IF admin_user_id IS NOT NULL THEN
    ...
    IF NOT user_exists THEN
      ...
      VALUES (admin_user_id, 'admin@leadflowapp.online', NOW(), NOW(), true, true, admin_user_id::text);
    ELSE
      -- Update admin user in public.users
      UPDATE public.users
      SET is_admin = true, is_active = true, updated_at = NOW(), token_identifier = id::text
      WHERE id = admin_user_id AND (token_identifier IS NULL OR token_identifier = '');
    END IF;
  ...
  IF test_user_id IS NOT NULL THEN
    ...
    IF NOT user_exists THEN
      ...
      VALUES (test_user_id, 'ibbysj@gmail.com', NOW(), NOW(), true, test_user_id::text);
    ELSE
      -- Update test user in public.users
      UPDATE public.users
      SET is_active = true, updated_at = NOW(), token_identifier = id::text
      WHERE id = test_user_id AND (token_identifier IS NULL OR token_identifier = '');
    END IF;
  ...
  LOOP
    ...
    IF NOT user_exists THEN
      -- Create user in public.users if not exists with token_identifier set to user id as text
      INSERT INTO public.users (id, email, created_at, updated_at, is_active, token_identifier)
      VALUES (auth_user_id, user_record.email, NOW(), NOW(), true, auth_user_id::text);
    ELSE
      -- Update user in public.users and ensure token_identifier is set
      UPDATE public.users
      SET is_active = true, updated_at = NOW(), token_identifier = id::text
      WHERE id = auth_user_id AND (token_identifier IS NULL OR token_identifier = '');
    END IF;
  ...
  UPDATE users SET token_identifier = id::text WHERE token_identifier IS NULL;
