...
-- Create or replace exec_sql function for admin operations
...
-- Create or replace get_total_deal_value function
CREATE OR REPLACE FUNCTION public.get_total_deal_value(stage_filter text DEFAULT NULL)
RETURNS numeric
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  IF stage_filter IS NULL THEN
    SELECT COALESCE(SUM(deal_value), 0) INTO total_value FROM deals;
  ELSE
    SELECT COALESCE(SUM(deal_value), 0) INTO total_value FROM deals WHERE stage = stage_filter;
  END IF;
