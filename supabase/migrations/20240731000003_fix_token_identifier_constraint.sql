-- Fix the token_identifier constraint issue
DO $$
BEGIN
  -- Check if the constraint exists and drop it
  IF EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'users_token_identifier_key') THEN
    ALTER TABLE public.users DROP CONSTRAINT users_token_identifier_key;
  END IF;
  ...
  BEGIN
    ...
    IF test_user_exists THEN
      ...
      -- Delete and recreate the user in public.users to avoid constraint issues
      DELETE FROM public.users WHERE email = 'ibbysj@gmail.com';
  ...
  BEGIN
    ...
    ELSE
      ...
      -- Delete and recreate the admin user in public.users to avoid constraint issues
      DELETE FROM public.users WHERE email = 'admin@leadflowapp.online';
